...
const WishlistCategories = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/api/admin/wishlist-categories/${editingId}`, formData);
      } else {
        await axios.post('/api/admin/wishlist-categories', {
          ...formData,
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this category? All products in it will be removed.')) return;
    try {
      await axios.delete(`/api/admin/wishlist-categories/${id}`);
      fetchCategories();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">Wishlist Categories</h2>
          <button
            type="button"
            onClick={openCreate}
              ...
              {categories.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-gray-500">
                    No categories yet. Add one to manage wishlist sections (e.g. Wishlist, Baskets, Gadgets, Perfume).
                  </td>
                </tr>
              ) : (
                categories.map((cat) => (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Wishlist, Baskets, Perfume"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. wishlist, baskets"
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-nex-orange focus:border-transparent"
                />
              </div>
              <div>
