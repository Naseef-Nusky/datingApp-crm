...
const Presents = () => {
  ...
  const categoryOptions = useMemo(() => {
    ...
    if (presentCategories.length > 0) {
      // Primary source: dynamic Present Categories (value = NAME)
      presentCategories.forEach((c) => {
        if (c.name) values.add(c.name);
      });
    } else {
      // Fallback: legacy default options when no dynamic categories exist yet
      defaultCategoryOptions.forEach((c) => values.add(c.value));
    }

    // Also include any categories already used by existing presents
    gifts.forEach((g) => {
  ...
  const fetchPresentCategories = async () => {
    try {
  ...
  useEffect(() => {
    fetchGifts();
    fetchPresentCategories();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const openCreate = () => {
    setEditingId(null);
    // Refresh categories so the dropdown includes any newly added present categories
    fetchPresentCategories();
    setFormData({
      name: '',
      description: '',
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-nex-orange focus:border-transparent text-sm bg-white"
                >
                  {categoryOptions.map((opt) => (
                    <option key={opt.value} value={opt.value}>
                      {opt.label}
                ...
                <p className="mt-1 text-[11px] text-gray-500">
                  To add new categories, use the <span className="font-semibold">Present Categories</span> page.
                </p>
              </div>
              <div>
